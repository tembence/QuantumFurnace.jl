...
mutable struct TrottTrott
    t0::Float64
    num_trotter_steps_per_t0::Rational{Int64}  #! FINISH ADDING RATIONAL NUMS
...
@printf("T: %f\n", T)
@printf("t0: %f\n", t0)
num_trotter_steps_per_t0 = Int(round(4 * T, digits=0))
num_trotter_steps = int_multiple * num_trotter_steps_per_t0
@time trott2_t0 = trotterize2(hamiltonian, t0, num_trotter_steps_per_t0)

@time trott2 = trotter2_diag(hamiltonian, T, num_trotter_steps)
dist2 = norm(trott2 - U_exact)
@printf("Distance 2nd order with diag: %s\n", dist2)

@time trotter = create_trotter(hamiltonian, t0, num_trotter_steps_per_t0)
@time trott_error = compute_trotter_error(hamiltonian, trotter, T)
